...
// Setup space key to play open triangle
function setupSpaceKeyTriangle() {
  var spacePressed = false;
  
  $(window).on('keydown.triangle', function(evt) {
    ...
    // Handle space key for open triangle
    if (evt.which === 32 && !spacePressed) { // 32 is space key
      spacePressed = true;
      // Play open triangle (note 81) on drum channel with velocity 90
      MIDI.noteOn(DRUM_CHANNEL, 81, 90);
      console.log('Playing open triangle');
    }
  });
  
  $(window).on('keyup.triangle', function(evt) {
    if (evt.which === 32) { // 32 is space key
      spacePressed = false;
      // Stop the open triangle sound
      MIDI.noteOff(DRUM_CHANNEL, 81);
    }
  });
}